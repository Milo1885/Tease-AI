...
ByVal lpstrReturnString As String, ByVal uReturnLength As Integer, ByVal hwndCallback As Integer) As Integer
    ...
    Private Sub Form1_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If My.Settings.DomLevel <> 0 Then ssh.domLevel = My.Settings.DomLevel

        If ssh.domLevel = 1 Then FrmSettings.DomLevelDescLabel.Text = "Gentle"
        If ssh.domLevel = 2 Then FrmSettings.DomLevelDescLabel.Text = "Lenient"
        If ssh.domLevel = 3 Then FrmSettings.DomLevelDescLabel.Text = "Tease"
        If ssh.domLevel = 4 Then FrmSettings.DomLevelDescLabel.Text = "Rough"
        If ssh.domLevel = 5 Then FrmSettings.DomLevelDescLabel.Text = "Sadistic"
        ...
        If My.Settings.GiveUpContinue = True Then
            FrmSettings.GiveUpContinueCheckBox.Checked = True
        Else
            FrmSettings.GiveUpContinueCheckBox.Checked = False
        End If
        ...
        ssh.orgasmAllow = My.Settings.OrgasmAllow
        ssh.orgasmRuin = My.Settings.OrgasmRuin
    ...
    Public Sub ResetButton()
        ...
        ssh.domLevel = My.Settings.DomLevel
        ssh.domEmpathy = My.Settings.DomEmpathy
            ...
            For i As Integer = 0 To SplitParts.Length - 1
                ...
                If UCase(ssh.ChatString).Contains(UCase(SplitParts(i))) Then
                    ...
                    If FrmSettings.CBTeaseLengthDD.Checked = True Then
                        ...
                        If ssh.domLevel = 1 Then ssh.TeaseTick = ssh.randomizer.Next(10, 16) * 60
                        If ssh.domLevel = 2 Then ssh.TeaseTick = ssh.randomizer.Next(15, 21) * 60
                        If ssh.domLevel = 3 Then ssh.TeaseTick = ssh.randomizer.Next(20, 31) * 60
                        If ssh.domLevel = 4 Then ssh.TeaseTick = ssh.randomizer.Next(30, 46) * 60
                        If ssh.domLevel = 5 Then ssh.TeaseTick = ssh.randomizer.Next(45, 61) * 60
...
NoPlaylistStartFile:
                    ...
                    ssh.LastScriptCountdown = ssh.randomizer.Next(3, 5 * ssh.domLevel)
        ...
        If ssh.EdgeFound = True And My.Settings.Chastity = False Then
            ...
            If ssh.SubEdging = True Then
                ...
                If ssh.SubStroking = True Then
                    ...
                    Else
                        ssh.AvgEdgeStroking = (ssh.AvgEdgeStroking + ssh.EdgeCountTick) / 2
                    End If
                    ...
                    'Else
                    '   ssh.AvgEdgeNoTouch = (ssh.AvgEdgeNoTouch + ssh.EdgeCountTick) / 2
                    '    End If
                ...
                If ssh.domLevel = 1 Then ssh.HoldEdgeChance = 20
                If ssh.domLevel = 2 Then ssh.HoldEdgeChance = 25
                If ssh.domLevel = 3 Then ssh.HoldEdgeChance = 30
                If ssh.domLevel = 4 Then ssh.HoldEdgeChance = 40
                If ssh.domLevel = 5 Then ssh.HoldEdgeChance = 50
                ...
                Else
                    ...
                    If ssh.OrgasmDenied = True Then
                        ...
                        If FrmSettings.CBDomDenialEnds.Checked = False And ssh.TeaseTick < 1 Then
                            ...
                            If RepeatChance < 10 * ssh.domLevel Then
                                ssh.SubEdging = False
                                ssh.SubStroking = False
                                EdgeTauntTimer.Stop()
...
RuinedOrgasm:
                ...
                If FrmSettings.CBDomOrgasmEnds.Checked = False And ssh.OrgasmRuined = True And ssh.TeaseTick < 1 Then
                    ...
                    If RepeatChance < 8 * ssh.domLevel Then
...
NoNoCumFiles:
                ...
                If FrmSettings.CBDomOrgasmEnds.Checked = False And ssh.TeaseTick < 1 Then
                    ...
                    If RepeatChance < 4 * ssh.domLevel Then
                        ssh.SubEdging = False
                        ssh.SubStroking = False
                        EdgeTauntTimer.Stop()
...
DebugAwarenessStep2:
        ...
        If ssh.CBTCockFlag = True Or ssh.CBTBallsFlag = True Or ssh.CBTBothFlag = True Or ssh.CustomTask = True Then
            ssh.TasksCount -= 1
            If ssh.TasksCount < 1 Or ssh.SubGaveUp Then
...
ModuleEnd:
        ...
        If File.Exists(CheckText) Then
            ...
            'line = ScriptLineVal For i As Integer = 
...
NonModuleEnd:
                ...
                If ssh.ReturnFlag = True Then
                    If ssh.numReturns = 0 Then
                        ssh.ReturnFlag = False
                    End If

                    ssh.FileText = ssh.ReturnFileText(ssh.numReturns)
                    ssh.StrokeTauntVal = ssh.ReturnStrokeTauntVal(ssh.numReturns)
                    ssh.ReturnSubState = ssh.ReturnState(ssh.numReturns)
                    ssh.ReturnFileText.RemoveAt(ssh.numReturns)
                    ssh.ReturnStrokeTauntVal.RemoveAt(ssh.numReturns)
                    ssh.ReturnState.RemoveAt(ssh.numReturns)
                    ssh.numReturns -= 1
            ...
            'FileGoto = "DommeLevel" & GotoDommeLevelString & ssh.domLevel
            ...
            'TempVal = randomizer.Next(5, 21) * ssh.domLevel

            ' 5, 20   10, 40   15, 60   20, 80   25, 100
            'If TempVal < 10 * ssh.domLevel Then
...
DommeSlideshowFallback:
                ...
                If ssh.SubGaveUp = True Then
                    ...
                    If ssh.ReturnFlag Then
                        ...
                        ssh.AskedToGiveUpSection = False
                        ScriptTimer.Start()
                    ElseIf ssh.TeaseTick < 1 And ssh.Playlist = False And ssh.BookmarkModule = False And ssh.BookmarkLink = False Then
                        ssh.StrokeTauntVal = -1
                        RunLastScript()
                    ElseIf WasStroking And Not WasEdging And Not WasHolding Then
                        ssh.StrokeTauntVal = -1
                        RunModuleScript(False)
                    ElseIf ssh.GiveUpContinue Then
                        ssh.ShowModule = True
                        ssh.AskedToGiveUpSection = False
                        ScriptTimer.Start()
                    Else
...
#End Region
    ...
    Public Sub CBTScript()
        ...
        CBTAmount = ssh.randomizer.Next(1, 6) * 2 * ssh.domLevel
...
StatusUpdateEnd:
    ...
    Public Function SysKeywordClean(ByVal StringClean As String) As String
        ...
        StringClean = StringClean.Replace("#DomLevel", ssh.domLevel)

        StringClean = StringClean.Replace("#DomApathy", ssh.domEmpathy)
        ...
        StringClean = StringClean.Replace("#DomOrgasmRate", LCase(ssh.orgasmAllow).Replace("allows", "allow"))

        StringClean = StringClean.Replace("#DomRuinRate", LCase(ssh.orgasmRuin).Replace("ruins", "ruin"))
        ...
        If StringClean.Contains("#RANDNumberLow") Then
            ...
            ssh.TempVal = ssh.randomizer.Next(1, 6) * ssh.domLevel
        ...
        If StringClean.Contains("#RANDNumberHigh") Then
            ...
            ssh.TempVal = ssh.randomizer.Next(5, 21) * ssh.domLevel
            If ssh.TempVal > 10 Then ssh.TempVal = 5 * Math.Round(ssh.TempVal / 5)
        ...
        If StringClean.Contains("#RANDNumber") Then
            ...
            ssh.TempVal = ssh.randomizer.Next(1, 11) * ssh.domLevel
            If ssh.TempVal > 10 Then ssh.TempVal = 5 * Math.Round(ssh.TempVal / 5)
            If ssh.TempVal < 3 Then ssh.TempVal = 3
...
TaskCleanSet:
        ...
        If StringClean.Contains("@DecreaseOrgasmChance") Then
            ...
            If ssh.orgasmAllow = "Rarely Allows" Then ssh.orgasmAllow = "Never Allows"
            If ssh.orgasmAllow = "Sometimes Allows" Then ssh.orgasmAllow = "Rarely Allows"
            If ssh.orgasmAllow = "Often Allows" Then ssh.orgasmAllow = "Sometimes Allows"
            If ssh.orgasmAllow = "Always Allows" Then ssh.orgasmAllow = "Often Allows"

            FrmSettings.alloworgasmComboBox.Text = ssh.orgasmAllow
        ...
        If StringClean.Contains("@IncreaseOrgasmChance") Then
            ...
            If ssh.orgasmAllow = "Often Allows" Then ssh.orgasmAllow = "Always Allows"
            If ssh.orgasmAllow = "Sometimes Allows" Then ssh.orgasmAllow = "Often Allows"
            If ssh.orgasmAllow = "Rarely Allows" Then ssh.orgasmAllow = "Sometimes Allows"
            If ssh.orgasmAllow = "Never Allows" Then ssh.orgasmAllow = "Rarely Allows"
        ...
        If StringClean.Contains("@DecreaseRuinChance") Then
            ...
            If ssh.orgasmRuin = "Rarely Ruins" Then ssh.orgasmRuin = "Never Ruins"
            If ssh.orgasmRuin = "Sometimes Ruins" Then ssh.orgasmRuin = "Rarely Ruins"
            If ssh.orgasmRuin = "Often Ruins" Then ssh.orgasmRuin = "Sometimes Ruins"
            If ssh.orgasmRuin = "Always Ruins" Then ssh.orgasmRuin = "Often Ruins"

            FrmSettings.ruinorgasmComboBox.Text = ssh.orgasmRuin
        ...
        If StringClean.Contains("@IncreaseRuinChance") Then
            ...
            If ssh.orgasmRuin = "Often Ruins" Then ssh.orgasmRuin = "Always Ruins"
            If ssh.orgasmRuin = "Sometimes Ruins" Then ssh.orgasmRuin = "Often Ruins"
            If ssh.orgasmRuin = "Rarely Ruins" Then ssh.orgasmRuin = "Sometimes Ruins"
            If ssh.orgasmRuin = "Never Ruins" Then ssh.orgasmRuin = "Rarely Ruins"
        ...
        If StringClean.Contains("@StartStroking") Then
            ...
            If FrmSettings.CBTauntCycleDD.Checked = True Then
                If ssh.domLevel = 1 Then ssh.StrokeTick = ssh.randomizer.Next(1, 3) * 60
                If ssh.domLevel = 2 Then ssh.StrokeTick = ssh.randomizer.Next(1, 4) * 60
                If ssh.domLevel = 3 Then ssh.StrokeTick = ssh.randomizer.Next(3, 6) * 60
                If ssh.domLevel = 4 Then ssh.StrokeTick = ssh.randomizer.Next(4, 8) * 60
                If ssh.domLevel = 5 Then ssh.StrokeTick = ssh.randomizer.Next(5, 11) * 60

                If ssh.WorshipMode = True Then
                    If ssh.domLevel = 1 Then ssh.StrokeTick = 180
                    If ssh.domLevel = 2 Then ssh.StrokeTick = 240
                    If ssh.domLevel = 3 Then ssh.StrokeTick = 360
                    If ssh.domLevel = 4 Then ssh.StrokeTick = 480
                    If ssh.domLevel = 5 Then ssh.StrokeTick = 600
                End If
        ...
        If StringClean.Contains("@DecideOrgasm") Then
            ...
            If ssh.orgasmAllow = "Always Allows" And ssh.orgasmRuin = "Always Ruins" Then
            ...
            If ssh.orgasmAllow = "Never Allows" Then OrgasmThreshold = 0
            If ssh.orgasmAllow = "Always Allows" Then OrgasmThreshold = 1000

            If FrmSettings.CBRangeOrgasm.Checked = True Then
                If ssh.orgasmAllow = "Rarely Allows" Then OrgasmThreshold = 20
                If ssh.orgasmAllow = "Sometimes Allows" Then OrgasmThreshold = 50
                If ssh.orgasmAllow = "Often Allows" Then OrgasmThreshold = 75
            Else
                If ssh.orgasmAllow = "Rarely Allows" Then OrgasmThreshold = FrmSettings.NBAllowRarely.Value
                If ssh.orgasmAllow = "Sometimes Allows" Then OrgasmThreshold = FrmSettings.NBAllowSometimes.Value
                If ssh.orgasmAllow = "Often Allows" Then OrgasmThreshold = FrmSettings.NBAllowOften.Value
            End If
            ...
            If ssh.orgasmRuin = "Never Ruins" Then RuinThreshold = 0
            If ssh.orgasmRuin = "Always Ruins" Then RuinThreshold = 1000


            If FrmSettings.CBRangeRuin.Checked = True Then
                If ssh.orgasmRuin = "Rarely Ruins" Then RuinThreshold = 20
                If ssh.orgasmRuin = "Sometimes Ruins" Then RuinThreshold = 50
                If ssh.orgasmRuin = "Often Ruins" Then RuinThreshold = 75
            Else
                If ssh.orgasmRuin = "Rarely Ruins" Then RuinThreshold = FrmSettings.NBRuinRarely.Value
                If ssh.orgasmRuin = "Sometimes Ruins" Then RuinThreshold = FrmSettings.NBRuinSometimes.Value
                If ssh.orgasmRuin = "Often Ruins" Then RuinThreshold = FrmSettings.NBRuinOften.Value
            End If
...
OrgasmDecided:
        ...
        If StringClean.Contains("@GiveUpCheck") Then
            ...
            Else
                ...
                If ssh.domEmpathy = 1 Then GiveUpCheck = 0
                If ssh.domEmpathy = 2 Then GiveUpCheck = 25
                If ssh.domEmpathy = 3 Then GiveUpCheck = 50
                If ssh.domEmpathy = 4 Then GiveUpCheck = 75
                If ssh.domEmpathy = 5 Then GiveUpCheck = 1000
        ...
        If StringClean.Contains("@Interrupt(") Then
            ...
            If File.Exists(Application.StartupPath & "\Scripts\" & dompersonalitycombobox.Text & "\Interrupt\" & InterruptS(0) & ".txt") Then
                ...
                If (ssh.ReturnFlag = True) Then
                    ssh.ReturnFlag = False
                    ssh.ReturnStrokeTauntVal = New List(Of Integer)
                    ssh.ReturnFileText = New List(Of String)
                    ssh.ReturnState = New List(Of String)
                    ssh.numReturns = -1
                End If
...
VTSkip:
        ...
        If StringClean.Contains("@SpeedUpCheck") Then
            ...
            Else
                ...
                Else
                    ...
                    If ssh.domLevel = 1 Then SpeedUpCheck = 70
                    If ssh.domLevel = 2 Then SpeedUpCheck = 40
                    If ssh.domLevel = 3 Then SpeedUpCheck = 60
                    If ssh.domLevel = 4 Then SpeedUpCheck = 50
                    If ssh.domLevel = 5 Then SpeedUpCheck = 65
        ...
        If StringClean.Contains("@CallReturn(") Then
            ...
            ssh.ReturnFlag = True
            GetSubState()
            ssh.ReturnFileText.Add(ssh.FileText)
            ssh.ReturnStrokeTauntVal.Add(ssh.StrokeTauntVal)
            ssh.ReturnState.Add(ssh.ReturnSubState)
            ssh.numReturns += 1
        ...
        If StringClean.Contains("@GotoDommeOrgasm") Then
            ...
            If ssh.orgasmAllow = "Never Allows" Then ssh.FileGoto = "(Never Allows)"
            If ssh.orgasmAllow = "Rarely Allows" Then ssh.FileGoto = "(Rarely Allows)"
            If ssh.orgasmAllow = "Sometimes Allows" Then ssh.FileGoto = "(Sometimes Allows)"
            If ssh.orgasmAllow = "Often Allows" Then ssh.FileGoto = "(Often Allows)"
            If ssh.orgasmAllow = "Always Allows" Then ssh.FileGoto = "(Always Allows)"
        ...
        If StringClean.Contains("@GotoDommeRuin") Then
            ...
            If ssh.orgasmRuin = "Never Ruins" Then ssh.FileGoto = "(Never Ruins)"
            If ssh.orgasmRuin = "Rarely Ruins" Then ssh.FileGoto = "(Rarely Ruins)"
            If ssh.orgasmRuin = "Sometimes Ruins" Then ssh.FileGoto = "(Sometimes Ruins)"
            If ssh.orgasmRuin = "Often Ruins" Then ssh.FileGoto = "(Often Ruins)"
            If ssh.orgasmRuin = "Always Ruins" Then ssh.FileGoto = "(Always Ruins)"
        ...
        If StringClean.Contains("@GotoDommeApathy") Then
            ...
            If ssh.domEmpathy = 1 Then ssh.FileGoto = "(ApathyLevel1)"
            If ssh.domEmpathy = 2 Then ssh.FileGoto = "(ApathyLevel2)"
            If ssh.domEmpathy = 3 Then ssh.FileGoto = "(ApathyLevel3)"
            If ssh.domEmpathy = 4 Then ssh.FileGoto = "(ApathyLevel4)"
            If ssh.domEmpathy = 5 Then ssh.FileGoto = "(ApathyLevel5)"
        ...
        If StringClean.Contains("@GotoDommeLevel") Then
            ...
            If ssh.domLevel = 1 Then ssh.FileGoto = "(DommeLevel1)"
            If ssh.domLevel = 2 Then ssh.FileGoto = "(DommeLevel2)"
            If ssh.domLevel = 3 Then ssh.FileGoto = "(DommeLevel3)"
            If ssh.domLevel = 4 Then ssh.FileGoto = "(DommeLevel4)"
            If ssh.domLevel = 5 Then ssh.FileGoto = "(DommeLevel5)"
...
SkipTextedTags:
    ...
    Public Function GetFilter(ByVal FilterString As String, Optional ByVal Linear As Boolean = False) As Boolean
        ...
        If FilterString.ToLower.Contains("@dommelevel1") And ssh.domLevel <> 1 Then Return False
        If FilterString.ToLower.Contains("@dommelevel2") And ssh.domLevel <> 2 Then Return False
        If FilterString.ToLower.Contains("@dommelevel3") And ssh.domLevel <> 3 Then Return False
        If FilterString.ToLower.Contains("@dommelevel4") And ssh.domLevel <> 4 Then Return False
        If FilterString.ToLower.Contains("@dommelevel5") And ssh.domLevel <> 5 Then Return False
        ...
        If FilterString.ToLower.Contains("@alwaysallowsorgasm") And ssh.orgasmAllow <> "Always Allows" Then Return False
        If FilterString.ToLower.Contains("@oftenallowsorgasm") And ssh.orgasmAllow <> "Often Allows" Then Return False
        If FilterString.ToLower.Contains("@sometimesallowsorgasm") And ssh.orgasmAllow <> "Sometimes Allows" Then Return False
        If FilterString.ToLower.Contains("@rarelyallowsorgasm") And ssh.orgasmAllow <> "Rarely Allows" Then Return False
        If FilterString.ToLower.Contains("@neverallowsorgasm") And ssh.orgasmAllow <> "Never Allows" Then Return False
        ...
        If FilterString.ToLower.Contains("@alwaysruinsorgasm") And ssh.orgasmRuin <> "Always Ruins" Then Return False
        If FilterString.ToLower.Contains("@oftenruinsorgasm") And ssh.orgasmRuin <> "Often Ruins" Then Return False
        If FilterString.ToLower.Contains("@sometimesruinsorgasm") And ssh.orgasmRuin <> "Sometimes Ruins" Then Return False
        If FilterString.ToLower.Contains("@rarelyruinsorgasm") And ssh.orgasmRuin <> "Rarely Ruins" Then Return False
        If FilterString.ToLower.Contains("@neverruinsorgasm") And ssh.orgasmRuin <> "Never Ruins" Then Return False

        If FilterString.ToLower.Contains("@notalwaysallowsorgasm") And ssh.orgasmAllow = "Always Allows" Then Return False
        If FilterString.ToLower.Contains("@notneverallowsorgasm") And ssh.orgasmAllow = "Never Allows" Then Return False
        If FilterString.ToLower.Contains("@notalwaysruinsorgasm") And ssh.orgasmRuin = "Always Ruins" Then Return False
        If FilterString.ToLower.Contains("@notneverruinsorgasm") And ssh.orgasmRuin = "Never Ruins" Then Return False
        ...
        If FilterString.Contains("@ApathyLevel1") And ssh.domEmpathy <> 1 Then Return False
        If FilterString.Contains("@ApathyLevel2") And ssh.domEmpathy <> 2 Then Return False
        If FilterString.Contains("@ApathyLevel3") And ssh.domEmpathy <> 3 Then Return False
        If FilterString.Contains("@ApathyLevel4") And ssh.domEmpathy <> 4 Then Return False
        If FilterString.Contains("@ApathyLevel5") And ssh.domEmpathy <> 5 Then Return False
    ...
    Public Function GetFilter2(ByVal FilterString As String) As Boolean
        ...
        Try
            ...
            With __ConditionDic
                ...
                .Add("@DommeLevel1", ssh.domLevel <> 1)
                .Add("@DommeLevel2", ssh.domLevel <> 2)
                .Add("@DommeLevel3", ssh.domLevel <> 3)
                .Add("@DommeLevel4", ssh.domLevel <> 4)
                .Add("@DommeLevel5", ssh.domLevel <> 5)
                ...
                .Add("@AlwaysAllowsOrgasm", ssh.orgasmAllow <> "Always Allows")
                .Add("@OftenAllowsOrgasm", ssh.orgasmAllow <> "Often Allows")
                .Add("@SometimesAllowsOrgasm", ssh.orgasmAllow <> "Sometimes Allows")
                .Add("@RarelyAllowsOrgasm", ssh.orgasmAllow <> "Rarely Allows")
                .Add("@NeverAllowsOrgasm", ssh.orgasmAllow <> "Never Allows")
                .Add("@AlwaysRuinsOrgasm", ssh.orgasmRuin <> "Always Ruins")
                .Add("@OftenRuinsOrgasm", ssh.orgasmRuin <> "Often Ruins")
                .Add("@SometimesRuinsOrgasm", ssh.orgasmRuin <> "Sometimes Ruins")
                .Add("@RarelyRuinsOrgasm", ssh.orgasmRuin <> "Rarely Ruins")
                .Add("@NeverRuinsOrgasm", ssh.orgasmRuin <> "Never Ruins")
                .Add("@NotAlwaysAllowsOrgasm", ssh.orgasmAllow = "Always Allows")
                .Add("@NotNeverAllowsOrgasm", ssh.orgasmAllow = "Never Allows")
                .Add("@NotAlwaysRuinsOrgasm", ssh.orgasmRuin = "Always Ruins")
                .Add("@NotNeverRuinsOrgasm", ssh.orgasmRuin = "Never Allows")
                ...
                .Add("@ApathyLevel1", ssh.domEmpathy <> 1)
                .Add("@ApathyLevel2", ssh.domEmpathy <> 2)
                .Add("@ApathyLevel3", ssh.domEmpathy <> 3)
                .Add("@ApathyLevel4", ssh.domEmpathy <> 4)
                .Add("@ApathyLevel5", ssh.domEmpathy <> 5)
...
PoundLoop:
            ...
            If TaskEntry.Contains("#TaskHours") Then
                Do
                    int = ssh.randomizer.Next(1, ssh.domLevel + 1) + ssh.domLevel
            ...
            If TaskEntry.Contains("#TaskMinutes") Then
                Do
                    int = ssh.randomizer.Next(5, 13) * ssh.domLevel
            ...
            If TaskEntry.Contains("#TaskSeconds") Then
                Do
                    int = ssh.randomizer.Next(10, 30) * ssh.domLevel * ssh.randomizer.Next(1, ssh.domLevel + 1)
            ...
            If TaskEntry.Contains("#TaskAmountLarge") Then
                Do
                    int = (ssh.randomizer.Next(15, 26) * ssh.domLevel) * 2
                    If int > 5 Then int = 5 * Math.Round(int / 5)
            ...
            If TaskEntry.Contains("#TaskAmountSmall") Then
                Do
                    int = (ssh.randomizer.Next(5, 11) * ssh.domLevel) / 2
                    If int > 5 Then int = 5 * Math.Round(int / 5)
            ...
            If TaskEntry.Contains("#TaskAmount") Then
                Do
                    int = ssh.randomizer.Next(15, 26) * ssh.domLevel
                    If int > 5 Then int = 5 * Math.Round(int / 5)
